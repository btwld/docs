...
description: "Define state-aware styles with variants for hover, press, dark mode, and more."
...
Suppose you want a `Box` to change its color when hovered. With Mix, you can achieve this by adding `.onHovered(...)` to your style definition:
...
When you define a style, it is expected to be reused in multiple places. You can easily reuse styles even when a variant was defined inside it:
...
You can combine multiple variants to achieve a specific style. For example, apply different colors for dark mode and light mode when the widget is hovered:
