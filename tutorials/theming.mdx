---
title: Theming
description: "Build a multi-theme application using design tokens and MixScope."
icon: "swatchbook"
---
...
In this guide, we will create a single-screen, multi-theme application to demonstrate the power of Mix's theming system.

## Setting up MixScope

Before styling your application with themes, configure `MixScope`. `MixScope` acts as an ancestor widget that passes down the defined tokens to all its child widgets, ensuring they can access and utilize the same styling information.

### Wrapping the root widget

To apply your theme globally, wrap your application's widgets with the `MixScope` widget:
...
Define your design tokens using Dart enums. This provides type safety, autocomplete support, and a clean API:
...
Map tokens to their actual values within theme classes:
...
The only custom component is a button:
...
1. **Token calling**: Call tokens as functions with `token()` to get token references
2. **TextStyle tokens**: Use `.token.mix()` for text style tokens
...
To toggle the theme, call `setState` to update `_isDarkPurpleTheme`. All widgets using tokens will automatically update.
