---
title: Creating context variants
description: "Apply styles conditionally based on runtime conditions from BuildContext."
icon: "code-branch"
---
...
Create a context variant that responds to a custom `InheritedWidget`:
...
Now create a `ContextVariant` that checks this inherited widget's state:
...
For better reusability and a cleaner API, create extension methods on `WidgetStateVariantMixin`:
...
Now use the extension method for a more readable API:
