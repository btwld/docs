...
description: "Create a button component with Specs, Stylers, variants, and code generation."
icon: "puzzle-piece"
---
...
No manual `copyWith`, `lerp`, or `props` needed. The generated mixin `_$ButtonSpecMethods` provides all of those. Run code generation with:

```bash
dart run build_runner build
```

<Expandable title="Generated code (button_spec.g.dart) — key signatures">
...
Use `@MixableField(setterType: ...)` to tell the generator the public type for each setter:
...
The `@MixableField(setterType: FlexBoxStyler)` annotation tells the generator to produce `ButtonStyler container(FlexBoxStyler value)` instead of `ButtonStyler container(StyleSpec<FlexBoxSpec> value)`.
...
<Expandable title="Generated code (button_style.g.dart) — key signatures">
...
- **`@MixableSpec`**: Generates `copyWith()`, `lerp()`, `debugFillProperties()`, and `props`
- **`@MixableStyler`**: Generates setters, `merge()`, `resolve()`, `debugFillProperties()`, and `props`
- **`@MixableField(setterType:)`**: Controls the public type of generated setters
- **`MixStyler`**: Base class that provides `WidgetStateVariantMixin`, `VariantStyleMixin`, and `AnimationStyleMixin`
